...
for path in files:
    ...
    for title, icon in icons.items():
        # Specifically match any SVG inside <div class="icon-tile">
        pattern = r'(<div class="icon-tile">)<svg.*?</svg>(</div>\s*<h3[^>]*>\s*(?:\d\.\s*)?' + re.escape(title) + r'\s*</h3>)'
        content = re.sub(pattern, r'\g<1>' + icon + r'\g<2>', content, flags=re.DOTALL)
        
    with open(path, 'w', encoding='utf-8') as f:
        f.write(content)

print('Updated icons properly!')
